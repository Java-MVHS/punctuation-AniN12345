 ...
 *     Call the `searchIt` method using the created instance.
 *   Inside the `searchIt` method:
 *     Create a variable called `word`.
 *     Use a for loop to iterate through the entire paragraph, identifying words by checking for spaces.
 *     For each identified word, pass it into the `getPunctuationWord` method.
 *   Inside the `getPunctuationWord` method:
 *     Pass the word into the `checkForPunctuation` method.
 *   Inside the `checkForPunctuation` method:
 *     Check if the word contains any punctuation.
 *     Return true if punctuation is found, otherwise return false.
 */
 ...
 public class Punctuation
 {
     ...
     {
         Punctuation p = new Punctuation();// Creates an instance of the class
         p.searchIt();	// Call searchit
     }
     
     public void searchIt()
     {
         System.out.print("\n\n\n"); // Adds spacing at the start of the output for formatting
         String word = new String(""); // Initialize the word variable to store each word
         String message = new String("Blood, Sweat, and Tears by Winston Churchill " +"May 13, 1940" + // This is the paragraph
         "Mr. Speaker:" +
         ...
         for(int i = 1; i < message.length(); i++) // Loop through each character in the paragraph
         {
             String character = new String(""); // Initialize character variable to hold a single character
             character = "" + message.charAt(i); // Get the current character from the paragraph
             word += character; // Append the character to the current word
             if (character.equals(" ") || i == message.length()-1) // Check if the character is a space or if we are at the end of the paragraph
             {
                 getPunctuationWords(word); // Pass the current word to the getPunctuationWords method
                 word = ""; // Reset the word variable
             }
         }
         System.out.print("\n\n\n"); // Adds spacing at the end of the output for formatting
     }
     public void getPunctuationWords(String userIn)
     {
         if(checkForPunctuation(userIn)) // Check if the word contains punctuation
             printWords(userIn); // Print the word if it contains punctuation
     }
     
     public boolean checkForPunctuation(String wordIn)
     {
         String punctuation = new String("()!\"',.:;?");		// These are the punctuation that we are checking each word for
         for (int i = 0; i < wordIn.length(); i++) // Loop through each character of the word
         {
             if (punctuation.indexOf(wordIn.charAt(i)) != -1) // Check if the character is a punctuation mark
                 return true; // Return true if punctuation is found
         }
         return false; // Return false if no punctuation is found
     }
     public void printWords(String userLn)
     {	
         System.out.print(userLn); // Print the word that contains punctuation
     }
 }
